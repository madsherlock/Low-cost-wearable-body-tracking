...
final static int SERIAL_PORT_BAUD_RATE = 1000000;
...
void setupRazor() {
  ...
  //serial.write("#t0#e0"); ////////////////////////////temperature off////////////////////////////////////////////
  serial.write("#t0#e1"); // #e1 means temperature readings on (first 4 bytes)
  ...
  pushMatrix(); {
    ...
    if (magnetomIndex < NUM_MAGN_SAMPLES && serial.available() >= 36) {
      ...
      while (serial.available() >= 40) {
        //Temperature data
        //skipBytes(serial,4);
        avgtemperature += readFloat(serial);
        temperaturecount++;
        // Skip accel data
        skipBytes(serial, 12);
        // Read magn data
...
void outputCalibration() {
  ...
  */
  ...
  avgtemperature /= float(temperaturecount);
  
  serial.write("#o0");
  delay(100);
  serial.clear();
  serial.write("#Mclit"); //get ID
  while(serial.available() < 2) ;
  char id0 = serial.readChar();
  char id1 = serial.readChar();
  System.out.printf(Locale.ENGLISH,"#Board ID: %c%c",id0,id1);

  System.out.printf(Locale.ENGLISH, "\n#Python calibration form:\nmagn_ellipsoid_center_x = %.12g\nmagn_ellipsoid_center_y = %.12g\nmagn_ellipsoid_center_z = %.12g\n",
	  center.get(0), center.get(1), center.get(2));
  System.out.printf(Locale.ENGLISH, "magn_ellipsoid_transform_0_0 = %.12g\nmagn_ellipsoid_transform_0_1 = %.12g\nmagn_ellipsoid_transform_0_2 = %.12g\n",
	  comp.get(0), comp.get(1), comp.get(2));
  System.out.printf(Locale.ENGLISH, "magn_ellipsoid_transform_1_0 = %.12g\nmagn_ellipsoid_transform_1_1 = %.12g\nmagn_ellipsoid_transform_1_2 = %.12g\n",
	  comp.get(3), comp.get(4), comp.get(5));
  System.out.printf(Locale.ENGLISH, "magn_ellipsoid_transform_2_0 = %.12g\nmagn_ellipsoid_transform_2_1 = %.12g\nmagn_ellipsoid_transform_2_2 = %.12g\n",
	  comp.get(6), comp.get(7), comp.get(8));
  System.out.printf(Locale.ENGLISH, "magn_working_temperature = %.12g\n",avgtemperature);
